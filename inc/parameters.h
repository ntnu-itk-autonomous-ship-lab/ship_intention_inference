...
struct IntentionModelParameters{
     ...
     */
    struct  {
        double min_time_cpa; /* How long before CPA we consider a situation to start */
    }set_startpoint;
    ...
    double ignoring_safety_probability; /* Whether or not the ship intends to keep a safe distance */
    double good_seamanship_probability; /* Prevents the ship from switching which side it is giving way towards */
    double unmodeled_behaviour; /* Any behaviour not captured by the model */
    ...
    IntentionModelParameters default_parameters(int num_ships){
        ...
        param.max_number_of_obstacles = num_ships-1; //must be set to num_ships-1 or else segmentation fault
        ...
        param.distance_midpoint_m.safe_mu = 300;
        param.distance_midpoint_m.safe_sigma = 75;
        param.distance_midpoint_m.risk_mu = 1000;
        param.distance_midpoint_m.risk_sigma = 200;
        param.distance_midpoint_m.max = 1800;
        param.distance_midpoint_m.n_bins = 30; // this value must match the bayesian network
        param.distance_front_m.safe_mu = 300;
        param.distance_front_m.safe_sigma = 75;
        param.distance_front_m.risk_mu = 1000;
        ...
        param.colregs_situation_borders_rad.OT_certain = 0.96; // 55 degrees
        param.colregs_situation_borders_rad.OT_uncertain = 1.40; // 80 degrees
        param.colregs_situation_borders_rad.HO_uncertain = 2.71; // 155 degrees
        param.colregs_situation_borders_rad.HO_certain = 2.97; // 170 degrees
        param.colregs_situation_borders_rad.U_uncertainty_m_s = 0.25;
        ...
        param.ignoring_safety_probability = 0.01;
